...
## 최근 내부 테스트 배포
- 2026-05-14: `1.0.38+58` Android internal track + iOS TestFlight beta 배포 진행. 지금 탭 약속 카드의 집중 타이머 ("지금 할게!" 버튼, 5/10/25분 또는 직접 입력, 종료 시 자동 "했어" 노트 prefill) 추가, 똑똑 수신 카드 UI 단순화 (별도 버튼 제거, 상단 배지 + 자동 lastPokeAcknowledgedAt 기록)를 포함한다.

...
## 최근 내부 테스트 배포
- 2026-05-30: `1.0.47+69` Android production + iOS App Store Connect(심사 제출) 배포 완료. 두 가지 UX 다듬기를 묶었다. (1) 약속 카드 진행 바 옆 텍스트가 `11/28`만 떠 있어 "분자가 성공일수인지 지난 일수인지 모호하다"는 피드백을 받아 `lib/widgets/plan/plan_card.dart`의 라벨을 상세 화면이 이미 쓰는 `planDetailProgressDoneLabel`("실천") + `planDetailProgressFractionDays`로 조합해 `실천 11/28일`(EN: `Done 11/28 days`)로 prefix 라벨링 — 새 l10n 키 없이 기존 두 문자열만 재사용. (2) 응원 리액션 이모지(`🔥❤️👏👍🙌🎉💪✨`)를 텍스트 글리프 대신 커스텀 SVG 아이콘으로 렌더링하도록 `lib/widgets/reaction_icon.dart`를 신설하고 `assets/icons/reactions/`에 8개 SVG를 추가. 저장/전송은 여전히 canonical emoji 문자열을 그대로 쓰고 매핑 없는 값은 기존 `CenteredEmoji`로 안전 fallback, `ReactionText`는 본문 문자열을 토큰 스캔해서 매치되는 부분만 인라인 SVG로 치환하므로 "고생했어 👏" 같은 메시지 내 부분 렌더링이 자연스럽다. 호출부(`now_tab_screen`의 응원 picker + `_PrimaryExecutorCard`의 statusMessage + `_buildDatedNoteBubble`, `action_note_dialog`의 picker, `history_card`의 코멘트 bubble) 일괄 전환 + picker 사이즈 22→28, 28→34로 살짝 키워 SVG 가시성 확보. 사전 검증으로 `flutter analyze` 통과(lib/ no issues — `test/`의 사전 `studyPlanTemplates`/`planCategories` 미정의 13건은 이전 빌드들과 동일하게 production AAB 빌드에 영향 없음). `fastlane release_all`로 production AAB → Play production 트랙 89초만에 업로드 완료(00:10:18), iOS는 precheck 통과 후 IPA 업로드 → 처리 완료 → `submit_for_review: true`로 App Store 심사 제출까지 자동(00:19:29). 전체 ~12분.
- 2026-05-27: `1.0.46+68` Android production + iOS TestFlight 배포 완료. 지금 탭 최상위 카드를 좌우 스와이프 캐러셀로 확장 — 사용자 피드백 ("두번째 할일을 먼저 하고 싶을 때 완료할 방법이 없다") 대응. `real_record_repository`가 종전에는 `upcomingItems.first` 1개만 `nowAction` 카드로 내보내고 나머지 다가올 항목은 버리고 있었는데, 오늘 남은 모든 upcoming 항목을 `nowAction`으로 emit하고 overdue도 3개 cap 제거하도록 변경. `NowTabState`에 `primaryCarouselCards: List<HomeCardModel>` 필드 추가하고 `_buildPrimaryCarousel`이 primary가 `nowAction`/`overdue`일 때 `[primary, ...나머지 upcoming, ...overdue]` 시간 오름차순으로 구성. `now_tab_screen.dart`는 primary 슬롯을 `GestureDetector(onHorizontalDragEnd: _handleCarouselSwipe)`로 감싸고 카드 아래에 `_PrimaryCarouselIndicator`(활성 점 18px, 비활성 6px) 노출, `_primaryActiveIndex` + signature 기반 리셋으로 캐러셀 구성이 바뀌면 첫 카드로 복귀, `_handleDidIt`/`_handleSkip`/`_handleRest`/`_handleFocusTimer`가 모두 `_activePrimaryCard()`로 라우팅돼 보이는 카드 기준으로 동작한다. 사전 검증으로 `flutter analyze`(touched 파일 no issues), `now_tab_viewmodel_test` 12개 모두 통과, build_runner로 freezed 재생성 확인까지 했으나 사용자 의사에 따라 수동 smoke는 스킵하고 바로 production AAB → Play production + iOS IPA → TestFlight beta 트랙에 업로드. iOS는 App Store Connect 처리 완료 후 내부 테스터 노출 대기.
